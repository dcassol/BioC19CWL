...
It's arguable if writing a help message is an overkill if the tool is only going to be executed inside a CWL wrapper. An alternative would be to use `commandArgs()` and parse the arguments yourself. One way to do it is described [here](https://www.r-bloggers.com/parse-arguments-of-an-r-script/).
