...
This workshop starts with teaching Common Workflow Language (**CWL**) basics before it shifts off to demonstrate exciting new CWL-related Bioconductor functionality to help R-savvy bioinformaticias see it from different perspectives. The `Rcwl` and `RcwlPipelines` packages provide a simple way to wrap command line tools and build CWL data analysis pipelines programmatically in R. Once the workflow is wrapped, with `systemPipeR` it's possible to run it from a single specification instance either entirely from within R, from various command-line wrappers (e.g., cwl-runner) or from other languages (e.g., Bash or Python). 
