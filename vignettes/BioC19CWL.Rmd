...
bibliography: "`r system.file(package='BioC19CWL', 'vignettes', 'bibtex.bib')`"
---
...
knitr::include_graphics(
            c(file.path(system.file(package='BioC19CWL', 'vignettes', "images"), "CWL-Logo.png"),
              "https://github.com/Bioconductor/BiocStickers/raw/master/systemPipeR/systemPipeR.png",
              "https://raw.githubusercontent.com/hubentu/BiocStickers/master/Rcwl/Rcwl.png"))
```
...
This workshop starts with teaching Common Workflow Language (**CWL**) basics before it shifts off to demonstrate exciting new CWL-related Bioconductor functionality to help R-savvy bioinformaticias see it from different perspectives. The `Rcwl` and `RcwlPipelines` packages provide a simple way to wrap command line tools and build CWL data analysis pipelines programmatically in R. Also, this workshop introduces how to use `systemPipeR's` new `CWL` interface to run data analysis workflows composed of both R and command-line analysis routines. We will provide an overview of the design of the `systemPipeR's` new CWL S4 class, while the hand-on components will cover the basic usage of CWL from both within R and the command-line, as well as the construction of custom workflows.

We will present a guide and best practices for creating container images (Docker) out of your R scripts, wrapping them in CWL and then sharing them with the wider world through the public apps gallery of the Cancer Genomics Cloud (**CGC**).

Participants will be guided through a self-contained R Markdown document. Participants are encouraged to try and create a CWL wrapper for a script on their own.
...
The Docker engine requirement is optional, but it can simplify software installation and management. <https://docs.docker.com/install/>
...
|----------------------------------------------|----------------|------|
| Introduction to `CWL`                        | Marko Zecevic  | 15m  |
| Turning your script into a command line tool | Marko Zecevic  | 15m  |
| `Rcwl` to wrap Bioinformatics tools          | Qiang Hu       | 10m  |
| Write your pipeline with `Rcwl`              | Qiang Hu       | 20m  |
| Case study                                   | Qiang Hu       | 10m  |
| Overview of `systemPipeR` package            | Daniela Cassol | 10m  |
| Workflow Control Class                       | Daniela Cassol | 05m  |
| Workflow Demonstration                       | Daniela Cassol | 25m  |
| Sharing your tools                           | Marko Zecevic  | 5m   |
...
Sharing a data analysis pipeline which you created using ad hoc methods is a challenging task. Failure to address any detail regarding commands, tools or the environment in which the analysis is performed can hinder the reproducibility of your work.

<br>
```{r, echo=FALSE, out.width="60%", fig.align='center'}
knitr::include_graphics(file.path(system.file(package='BioC19CWL', 'vignettes', "images"), "nocwl1.png"))
```

<div align="center">**Figure 1:** Prior modes of development: no containers (dependencies need to be harmonized), no standard workflow descriptions, executes differently in each environment. </div></br>
...
Applications described with Docker images and CWL will run consistently, in the way you intend, on everything from your local computer, your HPC, to your cloud infrastructure and various cloud platforms.

<br>
```{r, echo=FALSE, out.width="60%", fig.align='center'}
knitr::include_graphics(file.path(system.file(package='BioC19CWL', 'vignettes', "images"), "yescwl1.png"))
```

<div align="center">**Figure 2:** Development with standards: containers manage dependencies, standard workflow descriptions, executes the same in multiple environments. </div></br>

CWL can be written in easily readable and parsable YAML, or a bit less readable, but just as parsable JSON - or a combination of both! There’s an increasing number of extensions and tooling that facilitate code development, testing, and execution. One such set of tools is the [Rabix project](Rabix.io) from Seven Bridges - an open-source project that enables rapid composition and execution of CWL workflows in a manner that is scalable and reproducible. Rabix can be run locally to develop workflows on your own machine, or as a web based version that integrates directly with the Cancer Genomics Cloud (CGC) platform so you can develop and run your tools in the same place. Later on you will learn how to run CWL tools locally using SystemPipeR R package.

CWL as a standard consists of two separate specifications: CommandLineTool and Workflow. CommandLineTool describes how to execute a single command line (same as in the linux terminal), based on a set of input files and parameters. Workflow describes how multiple CommandLineTools and other Workflows are connected in an analysis.

Included in CWL CommandLineTool description:

* Class: `CommandLineTool`
* CWL version
* Tool label/id
* A list of inputs, outputs and a complete "recipe" for building the command line
* Reference to a container containing the tools with all dependencies
...
- id: output_filename
  ...
  inputBinding:
    ...
    valueFrom: |-
      ${
          return inputs.output_filename + ".bam"
      }
outputs:
- id: bam_output
  type: File?
...
<div align="center">**Figure 3:**  CommandLineTool example (Samtools view). </div></br>

Included in CWL Workflow description:

* Class: `Workflow`
* CWL version
* Workflow label/id
* A list of inputs and outputs
* A list of steps (individual tools) with connections

```
class: Workflow
cwlVersion: v1.0
label: SAMtools pipeline
id: samtools-1-6-pipeline
inputs:
  - id: input_sam_file
    type: File
  - id: output_filename
...
outputs:
  - id: indexed_bam
    outputSource:
      - samtools_1_6_index/indexed_bam
    type: File
  - id: bam_index
    outputSource:
      - samtools_1_6_index/bam_index
    type: File
steps:
  - id: samtools_1_6_view
    in:
      - id: input_sam_file
        source: input_sam_file
      - id: output_filename
        source: output_filename
    out:
      - id: bam_output
    run: samtools-view.cwl
  - id: samtools_1_6_sort
    in:
      - id: input_bam
        source: samtools_1_6_view/bam_output
    out:
      - id: sorted_bam
    run: samtools-sort.cwl
  - id: samtools_1_6_index
    in:
      - id: sorted_input
        source: samtools_1_6_sort/sorted_bam
    out:
      - id: indexed_bam
      - id: bam_index
    run: samtools-index.cwl
```
<div align="center">**Figure 4:**  Workflow example (Samtools view -> sort -> index). </div></br>

```{r, echo=FALSE, out.width="60%", fig.align='center'}
knitr::include_graphics(file.path(system.file(package='BioC19CWL', 'vignettes', "images"), "pipeline.png"))
```
<div align="center">**Figure 5:**  Graphical representation of a CWL workflow. </div></br>

Tool/workflow descriptions like these can also be made from within R using Rcwl package functionality.
...
Writing a help message could be considered an overkill if the tool is only going to be executed inside a CWL wrapper. An alternative would be to use `commandArgs()` and parse the arguments yourself. One way to do it is described [here](https://www.r-bloggers.com/parse-arguments-of-an-r-script/).
...
A Dockerfile consists of two kinds of items: instructions followed by arguments and comments. The first instruction and argument in our example Dockerfile assign a specific ubuntu 18.04 (revision identified by the `bionic-20190515` tag) image as the base image that we build upon. After that, each command adds another layer on top. And the base image we started from is probably also made of multiple layers. Think of layers as analogous to git commits - each layer is a set of filesystem changes. When you launch a container from an image, Docker adds a read-write layer to the top of that stack of read-only layers.
...
Building an image using the Dockerfile is really simple - just run the following command in the same directory you have placed your Dockerfile:

```{r, engine='bash', eval=FALSE}
docker build -t bioc2019cwl:0.1.0 .
```
...
compute clusters (Fig. 8). The latter supports interactive job submissions and batch
submissions to queuing systems of clusters. The main motivation and advantages
of using *`systemPipeR`* for complex data analysis tasks are:
...
<div align="center">**Figure 8:** Relevant features in *`systemPipeR`*.
Workflow design concepts are illustrated under (A & B). Examples of
*`systemPipeR’s`* visualization functionalities are given under (C). </div></br>
...
the following preconfigured directory structure (Fig. 9). Directory names are indicated
...
<div align="center">**Figure 9:** *systemPipeR's* preconfigured directory structure. </div></br>
...
10). This S4 class is a list-like container where each instance stores all the
...
<div align="center">**Figure 10:** Workflow steps with input/output file operations are controlled by 
...
or many *`SYSargs2`* objects are organized in a *`SYSargs2Pipe`* container. </div></br>
...
here next *`SYSargs2`* instance (see Fig. 10).
...
**Figure 11:** Up and down regulated DEGs identified by *`DESeq2`*. 
...
CWL is modular in nature: a CWL workflow consists of multiple steps which are nothing other than individual CWL tool descriptions linked together. These CWL tools are reusable and can be simply added to a new workflow every time you need them in your analysis. Because of this, tool developers are encouraged to wrap their tools into CWL and share these wrappers publicly, thus enabling scientists (and enthusiasts alike) to include them in their data analysis pipelines as easy as a simple drag and drop in a convenient GUI CWL editor.

<br>
```{r, echo=FALSE, out.width="60%", fig.align='center'}
knitr::include_graphics("https://github.com/rabix/composer/raw/master/doc/images/workflows.gif")
```
<br>
<div align="center">**Figure 12:** Creating CWL workflow with [Rabix Composer](https://github.com/rabix/composer). </div></br>

Said scientists can then do the same and make their pipelines publicly available as supplementary material to their research papers. A bundle of input data, a Docker image and a CWL description of the workflow will enable anyone interested in reproducing your results do that in a matter of few clicks.

One such place, where you can store your tools and data alongside secure and scalable computational resources is the Cancer Genomics Cloud (CGC), a cloud-based genomics analysis platform funded by the NCI and powered by Seven Bridges. This platform already hosts several massive public datasets[TCGA, TARGET, ICGC, …](https://docs.sevenbridges.com/docs/about-datasets) together with [a vast gallery of over 400 optimized, portable bioinformatics tools](https://cgc.sbgenomics.com/public/apps) which you can now add to! All of those tools, written in a variety of programming languages, are wrapped in CWL.

Publishing the tools and workflows you've developed to the Platform's Public Apps gallery is done by publishing the project which contains them. To do that, follow [these simple instructions](https://docs.cancergenomicscloud.org/docs/publish-your-app).

If you've been using Docker to encapsulate your tools (when describing in CWL using containers is actually not a hard requirement but a recommendation) - you should also consider registering your tools on the [Dockstore](https://dockstore.org/). It's an open platform for sharing Docker-based tools described in CWL or one of the two other description languages supported: WDL or Nextflow.
