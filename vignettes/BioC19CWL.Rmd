---
title: "Bridging Bioconductor to other bioinformatics tools using CWL"
date: "`r Sys.Date()`"
output:
...
rtool <- file.path(sysdir, "vignettes/exscrDoc.R")
...
library(BiocParallel)
ids <- c("normal", "tumor")
fq1 <- list.files(file.path(sysdir, "vignettes"), "1.fq.gz", full.names = TRUE)
fq2 <- list.files(file.path(sysdir, "vignettes"), "2.fq.gz", full.names = TRUE)
fq1 <- as.list(fq1)
fq2 <- as.list(fq2)
rg <- as.list(paste("@RG",
                    paste0("ID:", ids),
...
paramList <- list(threads = 2,
                  Ref = file.path(sysdir, "vignettes/ref.fa"))
